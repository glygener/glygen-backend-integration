...
def add_delta_list(bco_id, file_name_one, file_name_two, rel, prev_rel, out_json):
    ...
    in_file_two = data_release_dir + "v-%s/reviewed/%s" % (rel, file_name_two)
    if rel == current_rel:
...
def dump_bco_history(pair):
    ...
    out_json = {}
    in_file = current_jsondb_dir + "/historydb/record_count.csv"
    out_dir = current_jsondb_dir + "/historydb/"
...
def main():
    ...
    global current_count_file
    global current_rel
    global current_jsondb_dir

    current_rel = options.ver

    data_release_dir = "/data/shared/glygen/releases/data/"
    current_reviewed_dir = data_release_dir + "/v-%s/reviewed/" % (current_rel)
    current_jsondb_dir = data_release_dir + "/v-%s/jsondb/" % (current_rel) 
    current_count_file = data_release_dir + "/v-%s/jsondb/historydb/record_count.csv" % (current_rel)
    historydb_dir = data_release_dir + "/v-%s/jsondb/historydb/" % (current_rel)
    ...
    prev_idx = release_list.index(current_rel) - 1
    prev_rel = current_rel if prev_idx < 0 else release_list[prev_idx]
    pair = [prev_rel, current_rel]
    #print ("doing pair ", pair)
    #print (current_count_file)
    #exit()

    dump_record_counts(current_jsondb_dir, current_count_file, current_reviewed_dir)
    dump_bco_history(pair)
    print ("Finished legacy pair-releases (%s, %s)" % (prev_rel, current_rel))
