...
def main():
    ...
    log_file = "logs/make-alignmenetdb.%s-%s.log" % (start, end)
    msg = "make-alignmenetdb: started logging"
    ...
    for sheet_name in sheet_list:
        for species in species_list:
            ...
            if os.path.isfile(in_file) == False:
                continue
            msg = "make-alignmenetdb: processing %s" %(in_file)
    ...
    end = end if end < len(aln_file_list) else len(aln_file_list)
    

    cls_count = 0
    clsid2species = {}
    cls_id_list = []
    for aln_file in aln_file_list[start-1:end]:
        ...
        for seq_ac in aln_hash:
            if seq_ac != "consensus":
                ...
                if seq_ac in isoform2taxid:
                    ...
                    tax_name = species_obj[str(tax_id)]["common_name"]
    ...
    for obj in out_obj_list:
        ...
        out_file = "jsondb/alignmentdb/%s.json" % (obj["cls_id"])
        with open(out_file, "w") as FW:
            FW.write("%s\n" % (json.dumps(obj, indent=4)))
        record_count += 1 
    msg = "make-alignmenetdb: ... final created in: %s objects" % (record_count)
    csvutil.write_log_msg(log_file, msg, "a")
