...
def sort_release_list(tmp_list, reversed_flag):
    ...
    for rel in tmp_list:
        ...
        for i in range(0,len(parts)):
            parts[i] = 9 if parts[i] == "x" else parts[i]
...
def make_history_pair(pair):
    ...
    for bco_id in sorted(out_json):
        ...
        msg = "make-historydb: ... created record for %s" % (bco_id) 
...
def main():
    ...
    prev_rel = release_list[-2]
    pair = [prev_rel, current_rel]



    log_file = "logs/make-historydb.log"
    msg = "make-historydb: Started logging"
