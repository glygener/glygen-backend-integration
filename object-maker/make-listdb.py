...
def get_protein_record(obj):
    ...
    predicted_n_glycosites, predicted_o_glycosites = 0, 0
...
def get_motif_record(doc):
    ...
    protein_count =  len(list(seen.keys()))
...
def get_glycan_record(doc):
    ...
    if "species" in doc:
        for xobj in doc["species"]:
            #Merget Rattus to Rattus norvegicus
            name = "Rattus norvegicus" if xobj["name"] == "Rattus" else xobj["name"]
...
def main():
    ...
    for record_type in record_type_list:
        ...
        for json_file in file_list:
            ...
            record_count += 1
        print ("make-listdb-all: final created: %s %slist records" % (record_count, record_type))
