...
def main():
    ...
    for in_file in old_file_list:
        ...
        report_dict[file_name]["old"] = r_count
    ...
    for in_file in new_file_list:
        ...
        if file_name not in report_dict:
            report_dict[file_name] = {}
        report_dict[file_name]["new"] = r_count
        

    out_file = "logs/rowcount_changes.csv"
    FW = open(out_file, "w")
    row = ["rowcount_change", "rowcount_in_new", "rowcount_in_old", "file_name"]
    FW.write("%s\n"  % (",".join(row)))
