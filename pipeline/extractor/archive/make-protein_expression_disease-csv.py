...
def main():
    ...
    in_file = config_obj["pathinfo"]["downloads"] + "/bioxpress/bioxpress_disease.csv"
    ...
    #"uniprot_ac","refseq_ac","gene_name","log2fc","pvalue","adjpvalue","significance","direction","subjects_ratio","data_source","pmid","sample_name","do_id","do_name","uberon_id","sample_id"

    row = ["uniprot_canonical_ac","significance","direction", "doid", "doname", "parent_doid", "parent_doname"]
    print "\"%s\"" % ("\",\"".join(row))
    ...
    for ac in data_frame[prefix]:
        ...
        for obj in data_frame[prefix][ac]:
            if obj["doid"][0] in ["3963", "0070003", "3119"]:
                continue
            parent_doid, parent_doname = "", ""
            if len(obj["parent_doname"]) > 0:
                parent_doid, parent_doname = obj["parent_doid"][0], obj["parent_doname"][0]
            row = [canon, obj["significance"][0], obj["direction"][0],obj["doid"][0],obj["doname"][0], 
                    parent_doid, parent_doname]
            print "\"%s\"" % ("\",\"".join(row))
