...
"""
BCF Nov. 20, 2018
Updated now searches for glycosylation type in "intermediate" directory and writes final csv data object to file not screen
"""
######################
        ...
        for row in csv_grid:
            ...
            if row_count == 1:
                ...
                for j in xrange(1,len(field_list)):
                    ...
                    row_obj[field_name] = [] if row[j].strip() == "" else row[j].strip().replace("\"","").split("|")
                main_id = row[0].strip()
...
def main():
    ...
    tax_id = "9606" if species == "human" else "10090" #TODO support for other species


    data_frame = {}
    in_file = config_obj["pathinfo"]["downloads"] + "/unicarbkb/%s_glycosylation_current.csv" % (species)
    load_dataframe(data_frame, "glycosylation", in_file, ",")
    in_file = config_obj["pathinfo"]["intermediate"] + "/%s_glycosylation_types.csv" % (species)
    ...
    for uckb_id in uckbid2glycosylationtype:
        print uckb_id, "n-linked" in uckbid2glycosylationtype[main_id],  "o-linked" in uckbid2glycosylationtype[main_id]


    log_file = config_obj["pathinfo"]["intermediate"] + "/%s_proteoform_glycosylation_sites_unicarbkb_glytoucan.log" % (species)
    out_file = config_obj["pathinfo"]["unreviewed"] + "/%s_proteoform_glycosylation_sites_unicarbkb_glytoucan.csv" % (species)
    FWlog = open(log_file, "w")
    FWout = open(out_file, "w")
    ...
    for ac in data_frame["glycosylation"]:
        for obj in data_frame["glycosylation"][ac]:
            ...
            canon = ""
            tv =  [False, False, False, False, False, False,False]
