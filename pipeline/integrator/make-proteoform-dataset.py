...
def extract_glycosylation_sites_gptwiki_ds(species):
    ...
    out_df = {"fields":[], "data":[]}
    newrow = required_fields + ["glycopeptide_id", "composition", "glycan_xref_key","glycan_xref_id"]
    #print "\"%s\"" % ("\",\"".join(newrow))
    out_df["fields"] = newrow
...
def extract_glycosylation_sites_unicarbkb_glycomics_study_ds(species):
    ...
    data_frame = {}
    in_file = "downloads/unicarbkb/current/unicarbkb_glycomics_study.csv"
    libgly.load_sheet(data_frame, in_file, ",")
    f_list = data_frame["fields"]
    ...
    for row in data_frame["data"]:
        ...
        aa_three = aa_format_dict["one"][amino_acid] if amino_acid in aa_format_dict["one"] else ""
        flag_dict = {
        ...
        if uniprotkb_ac == "" or uniprotkb_ac not in ac2canon:
            ...
            canon = ac2canon[uniprotkb_ac]
            qc_glyco_position(position, amino_acid, seq_hash[canon], aa_format_dict, flag_dict)

    
        glycosylation_type = qc_glyco_type(position, amino_acid,saccharide,aa_format_dict,
                glytoucanac2glycosylationtype, flag_dict)
...
def extract_glycosylation_sites_unicarbkb_ds(species):
    ...
    extra_fields_dict = {
        "sarscov2":["glytoucan_structure","swiss_model","tissue_name","species","abundance",
            "pdb_id","cellosaurus","cell_line"]
    }
    out_df = {"fields":[], "data":[]}
    newrow = required_fields + ["unicarbkb_id", "composition","curation_notes", "additonal_notes"] 
    ...
    doid2name, uberonid2name = {}, {}
    in_file = "generated/misc/doid2uberonid-mapping.csv"
    data_frame = {}
    libgly.load_sheet(data_frame, in_file, ",")
    f_list = data_frame["fields"]
    for row in data_frame["data"]:
        doid2name[row[f_list.index("do_id")]] = row[f_list.index("do_name")]
        uberonid2name[row[f_list.index("uberon_id")]] = row[f_list.index("uberon_name")]

        
    #file_name = "all_glycosylation_current.csv"
    file_name = "unicarbkb_sarscov2.csv" if species in ["sarscov2"] else "unicarbkb_human_mouse_rat.csv" 
    data_frame = {}
    
    in_file = path_obj["downloads"] + "unicarbkb/current/" + file_name
    #in_file = path_obj["downloads"] + "unicarbkb/05_03_2021/" + file_name
    ...
    for row in data_frame["data"]:
        ...
        amino_acid = ""
        if "amino_acid" in f_list:
            amino_acid = row[f_list.index("amino_acid")].split("_")[-1] 
        if "aminoacid" in f_list:
        ...
        do_id = row[f_list.index("doid")].strip() if "doid" in f_list else ""
        uberon_id = row[f_list.index("uberon_id")].strip() if "uberon_id" in f_list else ""

        if do_id.find(":") != -1:
            do_id = do_id.split(":")[1]
        if uberon_id.find(":") != -1:
            uberon_id = uberon_id.split(":")[1]
        if do_id in doid2name:
            row[f_list.index("disease_name")] = doid2name[do_id]
        if uberon_id in uberonid2name:
            row[f_list.index("tissue_name")] = uberonid2name[uberon_id]
        ...
        if "doi" in f_list:
            ...
            if doi.strip() != "":
                pair_list.append(["protein_xref_doi", doi])
        additional_notes = row[f_list.index("additional_notes")] if "additional_notes" in f_list else ""
...
def qc_glyco_type(position, amino_acid,saccharide,aa_format_dict,glytoucanac2glycosylationtype, flag_dict):
...
def qc_glyco_position(position, amino_acid, canon_seq, aa_format_dict, flag_dict):
