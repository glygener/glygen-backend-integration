...
def extract_compiled_ds(ds_name, filter_flag):
    ...
    for row in data_frame["data"]:
        ...
        row_list = []
        if ds_name == "synthesized":
            xref_id = newrow[f_list.index("xref_id")]
            xref_key = "protein_xref_pubmed"
            row_list.append(row + [xref_id, xref_key])
            row_list.append(row + ["GLY_000309", "glycan_xref_glygen_ds"])
        else:
            row_list.append(row)
...
def extract_xrefs_ds(dataset):
    ...
    if dataset == "xref_glytoucan":
        ...
        if dataset == "xref_sandbox":
            in_file = path_obj["downloads"] + "glytoucan/current/export/glycotree_sandbox_accessions.tsv"
            libgly.load_sheet(data_frame, in_file, "\t")
        else:
        ...
        for row in data_frame["data"]:
            ac = row[0]
            xref_id = ac if dataset in ["xref_sandbox", "xref_gnome"] else row[1]
            if xref_id == "":
                continue
            if ac not in glycan_list:
                continue
...
def extract_enzyme_ds():
    ...
    data_frame = {}
    in_file = path_obj["downloads"] + "glytoucan/current/export/glycotree_annotated_glycans.tsv"
    libgly.load_sheet(data_frame, in_file, "\t")
    #in_file = path_obj["downloads"] + "uga/current/annotated_glycans.csv"
    #libgly.load_sheet(data_frame, in_file, ",")
    ...
    for row in data_frame["data"]:
        ...
        newrow = [
            ...
            val_dict["gene_name"],val_dict["gene_id"],val_dict["parent_residue_id"],val_dict["type"],
...
def extract_enzyme_ds_old():
    ...
    # [ GlyTouCanAccession,Enzyme_UniProtAccession, Enzyme_GeneName, Enzyme_GeneID, 
    #   Enzyme_TaxonomyID ]
...
def extract_xref_mapping_ds(ds_name):
    ...
    seen_row = {}
    if ds_name in ["glygen_uniprotkb_xref_mapping", "glygen_pubchem_xref_mapping"]:
        glyco_stats = get_glyco_stats([])
...
def extract_xref_chebi_ds():
    ...
    #in_file = path_obj["downloads"] + "chebi/chebiId_inchi_current.tsv"
    ...
    #for such rows, "COMPOUND_ID" is chebi_id and "ACCESSION_NUMBER" is glytoucan_ac.
    #Check the glytoucan_ac against the glycan_masterlist.csv. ** 
    ...
    with open(in_file, "r") as FR:
        for line in FR:
            line = line.strip()
            cid, chebi_id_raw = line.split("\t")
            if cid in cid2glytoucan and chebi_id_raw[0:6] == "CHEBI:":
                chebi_id = chebi_id_raw[6:]
                glytoucan_ac = cid2glytoucan[cid]
                if chebi_id in chebi2inchi and cid in pubchem2inchi:
                    chebi_inchi = chebi2inchi[chebi_id]
                    pubchem_inchi = pubchem2inchi[cid]
                    xref_key = "glycan_xref_chebi"
                    newrow = [glytoucan_ac, chebi_id, xref_key]
                    print "\"%s\"" % ("\",\"".join(newrow))
...
def extract_homolog_alignments_ds():
    ...
    for cls in seq_set:
        ...
        aln_file = "alignments/homologset.%s.aln" % (cls)
        seq_buffer_old = ""
...
def extract_glytoucan_linkout_ds():
    ...
    lines = commands.getoutput("cat unreviewed/glycan_sequences_wurcs.csv").split("\n")[1:]
    print "\"id\",\"sequence\""
    for line in lines:
        print line
...
def extract_uniprotkb_accession_history_ds():
    ...
    print "\"%s\"" % ("\",\"".join(newrow))
    for in_file in glob.glob(path_obj["downloads"] + "ebi/current/accession-history-*.csv"):
        data_frame = {}
        libgly.load_sheet(data_frame, in_file, ",")
        f_list = data_frame["fields"]
        for row in data_frame["data"]:
            old_ac = row[f_list.index("old_accession")]
            current_ac = row[f_list.index("current_accession")]
...
def main():
    ...
    if dataset == "masterlist":
        ...
        extract_monosaccharide_composition_ds()
            ...
            "sequences_glycam_iupac", "sequences_byonic"]:
    ...
    if dataset == "homolog_clusters":
        ...
        extract_ncbi_gene_linkouts_ds()
            ...
            "glygen_uniprotkb_xref_mapping", "glygen_iptmnet_xref_mapping"]:
    ...
    log_file = "logs/usage-%s.log" % (dataset)
