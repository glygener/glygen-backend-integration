...
def get_field_limits(dbh, limits_info):
...
def get_site_search_init(dbh, collection):
...
def get_idmapping_search_init(dbh):
...
def get_usecases_search_init(dbh):
    res_obj = {
    ...
    query_dict = {
    "species_to_glycosyltransferases":{"keywords":{"$eq":"glycosyltransferase-activity"}},
    "species_to_glycohydrolases":{"keywords":{"$eq":"glycohydrolase-activity"}},
    "species_to_glycoproteins":{"keywords":{"$eq":"glycoprotein"}}
    }
    
    prj_obj = {"species":1, "keywords":1}
    for svc in query_dict:
        for doc in dbh["c_protein"].find(query_dict[svc], prj_obj):
...
def get_protein_search_init(dbh, collection):
    ...
    res_obj = {"protein_mass":{}, "organism":[], "glycosylation_types":[]}
    min_mass = 10000000.0
    max_mass = -min_mass

    seen = {"protein_mass":{}, "organism":{}, "amino_acid":{}, "glycosylation_types":{}}
    for obj in dbh[collection].find({}):
...
def main():
        ...
        try:
            ...
            print "Getting get_idmapping_search_init ... "
            out_obj["idmapping"] = get_idmapping_search_init(dbh)
            print "done!"

            print "Getting usecases_search_init ... "
            out_obj["usecases"] = get_usecases_search_init(dbh)
            print "done!"

            print "Getting get_glycan_search_init ... "
            out_obj["glycan"] = get_glycan_search_init(dbh, "c_glycan")
            print "done!"

            print "Getting get_site_search_init ... "
            out_obj["site"] = get_site_search_init(dbh, "c_site")
            print "done!"


            print "Getting super_search_init ... "
            out_obj["supersearch"] = get_super_search_init(dbh)
            print "done!"



            print "Getting field limits ... "
            out_obj["fieldlimits"] = get_field_limits(dbh, config_obj["fieldlimitinfo"])
            print "done!"

            
            print "Getting get_protein_search_init ... "
