...
def get_glycan_record(doc, record_dict, seen):
    ...
    if sec in doc:
        for sec_obj in doc[sec]:
            tissue_obj = sec_obj["tissue"]
            cellline_obj = sec_obj["cell_line"]
            tissue_obj_str = json.dumps(tissue_obj)
            cellline_obj_str = json.dumps(cellline_obj)
            for ev_obj in sec_obj["evidence"]:
                record_id = ""
                ...
                if record_id != "" and record_id in record_dict:
                    if record_id not in seen:
                        seen[record_id] = {}
                    if "tissue" not in record_dict[record_id]:
                        record_dict[record_id]["tissue"] = []
                    if tissue_obj != {} and tissue_obj_str not in seen[record_id]:
                        record_dict[record_id]["tissue"].append(tissue_obj)
                        seen[record_id][tissue_obj_str] = True
                   
                    if "cell_line" not in record_dict[record_id]:
                        record_dict[record_id]["cell_line"] = []
                    if cellline_obj != {} and cellline_obj_str not in seen[record_id]:
                        record_dict[record_id]["cell_line"].append(cellline_obj)
                        seen[record_id][cellline_obj_str] = True
...
def main():
    ...
    for record_type in ["protein", "glycan"]:
        #file_list = glob.glob("jsondb/%sdb/G57321FI.json" % (record_type))
    ...
    for record_id in record_dict:
        ...
        with open(out_file, "w") as FW:
            FW.write("%s\n" % (out_str))
        record_count += 1
    print " ... final created: %s publication records" % (record_count)
