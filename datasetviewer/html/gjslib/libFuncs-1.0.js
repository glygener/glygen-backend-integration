...
rndrHeaderRowTwo: function (configObj, containerId){
	...
	for (var i=0; i < configObj.length; i++){
		cn += '<td width=60>';
		cn += '<img src="' + configObj[i]["href"] + '" width=50>';
	}
	cn += '</td><td class=hivelogo>';
	cn += '<u>H</u>igh performance <u>I</u>ntegrated <u>V</u>irtual <u>E</u>nvironment</td></tr>';
	cn += '</table>';
...
rndrHeaderRowFour: function (configObj, containerId){
	...
	for (var i=0; i < configObj.length; i++){
		cn += '<a href="' + configObj[i]["href"] + '" class=';
		cn += configObj[i]["class"] + '>' + configObj[i]["label"];
		cn += (i < configObj.length - 1 ? '</a> >> ' : '</a>');
	}
	$(containerId).html(cn);
	return;
},
...
rndrFooter: function (configObj, containerId){
	...
	for (var i=0; i < configObj.length; i++){
		cn += '<td class=rightbordered width=90>&nbsp; <a class="' + configObj[i]["class"];
