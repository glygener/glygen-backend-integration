...
rndrHeaderRowOne: function (configObj, containerId){
	...
	var cn = '<DIV style="position:relative;width:500px;height:60px;border:0px solid;">';
	for (var i=0; i < configObj.length; i++){
		cn += '<DIV class=' + configObj[i]["class"] + '>';
		cn += '<img src="' + configObj[i]["href"] + '" height=100%></DIV>';
	}
	cn += '</DIV>';
...
rndrHeaderRowTwo: function (configObj, containerId){
	...
	var cn = '<table cellpadding=0 style="background:#f1f1f1;" cellspacing=0 border=0>';
	cn += '<tr>';
	for (var i=0; i < configObj.length; i++){
		var itemType1 = (configObj[i]["label"].length < 1) ? 
				'<img src="' : '';
		var itemType2 = (configObj[i]["label"].length < 1) ? '">' : '';
		cn += '<td><a href="' + configObj[i]["href"] + '" class='+configObj[i]["class"]+'>';
		cn += itemType1 + configObj[i]["url"] + itemType2 + configObj[i]["label"] + '</a></td>';
	}
	cn += '</tr></table>';
...
rndrHeaderRowThree: function (configObj, containerId){
	...
	for (var i=0; i < configObj.length; i++){
		if (configObj[i]["class"] != 'regular13'){
	...
	for (var i=partition; i < configObj.length; i++){
		cn += '<a href="' + configObj[i]["href"] + '" class=';
		cn += configObj[i]["class"];
		cn += '>' + configObj[i]["label"] + '</a>';
		cn += (i < configObj.length - 1? ' &nbsp;|&nbsp; ' : '');
	}
...
/////////////////////////////
jsonToHtmlTableNew: function (jsonObj, containerId, styleJsonObj){

	if (jsonObj["output"].length == 1){
		var table = '<table width=90% height=300><tr><td valign=middle align=center>' +
			jsonObj["output"][0] + '</td></tr></table>';
		$('#'+containerId).html(table);
		return;
	}
	...
	function drawTable(){
		...
		for (var j=0; j < jsonObj["output"][0].length; j++){
			data.addColumn('string', jsonObj["output"][0][j]);
		}
		for (var i=1; i < jsonObj["output"].length; i++){
			data.addRows([jsonObj["output"][i]]);
		}
...
styleHash: function (tableStyleIndex, figureStyleIndex){
	...
	var tableStyleList = [
		...
		"columnType":["number","string","string","number","string","string","string"]
		},
